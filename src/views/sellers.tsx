    ...
    <div className="container mx-auto px-4 py-8">
      <div className="text-center mb-12">
        ...
        <p className="mt-4 text-lg text-muted-foreground max-w-2xl mx-auto">
          Profesionales inmobiliarios con experiencia para ayudarte a encontrar
      ...
      {showOnlyFavorites &&
        sellers.filter((seller) => favorites.includes(seller.user_id))
          .length === 0 && (
          <div className="text-center py-12">
            <Heart className="h-12 w-12 mx-auto text-muted-foreground" />
            <h3 className="mt-4 text-xl font-medium">
              No tienes vendedores favoritos
            </h3>
            <p className="mt-2 text-muted-foreground">
              Marca algunos vendedores como favoritos para verlos aquí
            </p>
            <Button
              variant="outline"
              className="mt-4"
      ...
      {sellers.filter((seller) =>
        showOnlyFavorites ? favorites.includes(seller.user_id) : true
      ).length > 0 && (
              ...
              >
                  ...
                  <CardDescription className="flex items-center">
                    ...
                    {(() => {
                      if (seller.topPropertyTypes.length === 1) {
                        return `${t("expert")} ${t("in")} ${
                          propertyTypeTranslation[
                            seller.topPropertyTypes[0].type as PropertyTypes
                          ]
                        }`;
                        ...
                        const types = seller.topPropertyTypes
                          .map(
                            (tp: PropertyTypes) =>
                              propertyTypeTranslation[tp.type as PropertyTypes]
                          )
                          .join(` ${t("and")} `);
                ...
                <CardContent className="flex-grow">
                  <div className="space-y-3 text-sm">
                    <div className="flex items-center">
                      ...
                      <span>
                        {t("zone")}: {seller.neighborhood || t("not_specified")}
                      </span>
                    </div>
                    <div className="flex items-center">
                ...
                <CardFooter className="border-t pt-4 flex flex-col items-start space-y-2">
                  ...
                  <div className="flex w-full gap-2 mt-2">
                    <Button
                      className="flex-1"
                      onClick={() => router.push(`/seller/${seller.user_id}`)}
                    >
                      {t("contact")}
                    </Button>
                    {isAuthenticated && (
                      <Button
                        variant={
                          favorites.includes(seller.user_id)
                            ? "default"
                            : "outline"
                        }
                        size="icon"
